...
using ngraph::op::PriorBoxClusteredIE;

namespace PlaidMLPlugin {

void registerPriorBoxClusteredIE() {
  registerOp("PriorBoxClusteredIE", [](const Context& ctx) {
    auto* layer = ngraph::as_type<ngraph::op::PriorBoxClusteredIE>(ctx.layer);
    IE_ASSERT(layer);
    ...
    auto input0_shape = ctx.layer->get_input_shape(0);
    auto input1_shape = ctx.layer->get_input_shape(1);
    int64_t layer_height = static_cast<int64_t>(input0_shape[2]);
    int64_t layer_width = static_cast<int64_t>(input0_shape[3]);
    int64_t img_height = static_cast<int64_t>(input1_shape[2]);
    int64_t img_width = static_cast<int64_t>(input1_shape[3]);
