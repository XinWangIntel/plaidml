...
#include "pmlc/target/nvidia_gpu/pass_detail.h"
#include "pmlc/target/nvidia_gpu/passes.h"
...
void pipelineBuilder(OpPassManager &pm) {
  ...
  pm.addPass(createNvidiaGpuLowerAffinePass());
  pm.addPass(createCanonicalizerPass());
  pm.addPass(createCSEPass());
...
static PassPipelineRegistration<>
    passPipelineReg("target-nvidia_gpu", "Target pipeline for Nvidia Gpus",
                    pipelineBuilder);

static compiler::TargetRegistration targetReg("nvidia_gpu", pipelineBuilder);
